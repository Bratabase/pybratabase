...
def test_fetch_index():
    ...
    assert sess.root.self == API_HOST, sess.root.self


def test_brands():
    sess = Session(host=API_HOST)
    brands = sess.root.links.brands.collection
    assert len(brands) == 20


def test_brand_detail():
    sess = Session(host=API_HOST)
    brands = sess.root.links.brands
...
def test_rate_limit():
    sess = Session(host=API_HOST)
    rate_limit = sess.root.links.rate_limit
...
def test_client_id():
    ...
    rate_limit = anon_sess.root.links.rate_limit.body['base']
    anon_total = rate_limit['limit'] + rate_limit['remaining']
    auth_sess = Session(host=API_HOST, credentials={
        'app_key': APP_ID,
        'app_secret': APP_SECRET
    })
    auth_rate_limit = auth_sess.root.links.rate_limit.body['base']
